...
* Because you are trying to fetch a user that isn't logged in.
...
* Describe the format of the data being sent.
...
*  Switch statement saying if this not this than this.  So if the url isn't present go to tbd  
...
* So you can come back to the app and have record of logging in.
...
* In our auth.js file since that is where our token is being generated by the login() function.
...
* Setting the current user ID to state allows the app to connect to the journal section of the app based on login ID or authorized user.

* **Question:** What happens if you enter in the incorrect information? What _should_ happen?

* **Your Answer:**

* If the information doesn't match user data from the dataBase then that user shouldn't be authorized to login.  Should receive an unauthorized users.
...
* You are adding a promise that the component did mount because you're authorizing a user ID with the database set of current stored users.  You are also setting the logged in user to state.
...
* We do not have a logout function set to handle the clearing of UserID in state or localStorage.
